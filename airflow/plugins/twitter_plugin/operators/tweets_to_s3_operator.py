...
class TweetsToS3Operator(BaseOperator):
    ...
    :param topic:               The subject to get tweets about.
    :type topic:                string
    ...
    def get_tweets(self, api):
        ...
        if not self.topic:
            raise ValueError("Topic is None. Cannot query nothing.")
        ...
        while len(tweets) < self.max_tweets:
            ...
            try:
                new_tweets = api.search(q=self.topic, count=count, max_id=str(last_id - 1))
                if not new_tweets:
                    break
    ...
    def execute(self, context):
        ...
        with NamedTemporaryFile("wb") as tmp:
            ...
            tweet_results = self.get_tweets(api)
